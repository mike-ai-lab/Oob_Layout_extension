...
// Fix for loading saved presets from Ruby
...
// Example: add to the payload sent to Ruby when applying the layout
// (You must integrate this with the function that collects all dialog values and sends to Ruby)

// Example usage:
// let adv = getOobAdvancedControls();
// window.location = 'skp:applyLayout@' + JSON.stringify(adv);

// --- Populate Starting Row Height options dynamically (if needed) ---
function setRowHeightOptions(heights) {
	const sel = document.getElementById('startRowHeight');
	if (!sel) return;
